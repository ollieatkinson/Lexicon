...
public extension JSON {
	...
	static func decoded<Value: Decodable>(
		...
		try JSONDecoder().decode(Value.self, from: value.data())
	}
...
private extension JSON {
	...
	static func number(_ value: NSNumber) -> JSON {
		...
		#else
		if ["B", "c"].contains(String(cString: value.objCType)) {
			return JSON(value.boolValue)
		}
		#endif
	...
	static func signedNumber(_ value: NSNumber) -> JSON {
	...
	static func floatingPoint(_ value: Double) -> JSON {
